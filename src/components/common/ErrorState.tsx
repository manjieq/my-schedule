  ...
  return (
    <Animated.View entering={FadeIn.duration(180)} exiting={FadeOut.duration(140)}>
      <View className="mx-4 mb-2 flex-row items-center gap-3 rounded-xl bg-red-50 px-4 py-3 dark:bg-red-950">
        ...
        {onRetry ? (
          <Pressable onPress={onRetry} hitSlop={10} android_ripple={ripple} className="rounded-md px-1 py-0.5">
            <Text className="text-sm font-semibold text-red-700 dark:text-red-300">Retry</Text>
          </Pressable>
        ) : null}
        {onDismiss ? (
          <Pressable onPress={onDismiss} hitSlop={10} android_ripple={ripple} className="rounded-md px-1 py-0.5">
